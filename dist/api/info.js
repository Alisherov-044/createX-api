...
exports.infoRouter
    .get("/", (_, res) => {
    (0, database_1.getAllData)("info", (error, data) => {
    ...
    try {
        scheme.parse(info);
        (0, database_1.createData)("info", info, (error) => {
            if (error) {
                return res
...
exports.infoRouter
    ...
    (0, database_1.getDataById)("info", { id }, (error, data) => {
        if (error || typeof data === "undefined") {
            return res.json({ message: "not found" }).status(404);
        }
        res.json(data).status(200);
    ...
    .delete("/:id", (req, res) => {
    const id = req.params.id;
    (0, database_1.deleteData)("info", { id }, (error) => {
        if (error) {
            return res
                .json({ message: "error accured while deleting info" })
                .status(403);
        }
        res.json({ message: `${id} - info delted successfully` }).status(400);
    });
});
